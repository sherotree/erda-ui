...
export interface IProps extends CONFIG_PAGE.ICommonProps {
  data: { boards: IData[]; refreshBoard?: boolean };
  props: {
    visible: boolean;
    isLoadMore: boolean;
  };
...
const IssueKanban = (props: IProps) => {
  ...
  const [boards, setBoard] = React.useState(data?.boards || []);
  ...
  React.useEffect(() => {
    if (!isLoadMore || data?.refreshBoard) {
      ...
      setBoard((prev) =>
        map(prev, (item) => {
          const curNewData = find(data?.boards, (newItem) => newItem.labelKey === item.labelKey);
  ...
  const doAdd = () => {
    const existKanban = find(boards || [], { label: addValue });
    if (existKanban) {
  ...
  const labelList = map(boards || [], 'label');
  ...
  return (
    <ScrollingComponent className="dice-cp issue-kanban">
      ...
      {isEmpty(data?.boards || []) ? <EmptyHolder relative className="w-full" /> : null}
    </ScrollingComponent>
  );
};
...
const Kanban = (props: IKanbanProps) => {
  ...
  const { title: label, id: labelKey, cards: propsList, total, pageSize, pageNo, operations: boardOp } = data;
  ...
  const changeData = (item: any) => {
    const { id, title, operations, userID, extra } = item;
    const { type, priority, status } = extra;
