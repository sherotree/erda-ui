...
const chartArr = [
  {
    title: `${i18n.t('msp:number of interface calls')}`,
    type: 'qps',
    i: 'register-qps',
  },
  {
    title: `${i18n.t('msp:number of failed calls')}`,
    type: 'failed',
    i: 'register-failed',
  },
  {
    title: `${i18n.t('msp:average call time')}`,
    type: 'avgtime',
    i: 'register-avgtime',
  },
];
  ...
  return (
    <div>
      <div className="interface-register-time mb-5">
        <p className="mb-3">{`Provider ${i18n.t('msp:register time')}：${moment(dubboDetailTime.providerTime).format(
          'YYYY-MM-DD HH:mm:ss',
        )}`}</p>
        <p>{`Consumer ${i18n.t('msp:register time')}：${moment(dubboDetailTime.consumerTime).format(
          'YYYY-MM-DD HH:mm:ss',
        )}`}</p>
      </div>
