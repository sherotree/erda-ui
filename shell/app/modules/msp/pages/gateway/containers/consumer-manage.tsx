...
import { Button, Modal, Drawer, Table, Select, Popover, Spin, Tabs } from 'core/nusi';
...
export const PureConsumerManage = () => {
  ...
  const [openApiConsumerList, paging, consumerAuthPackages, authConfig] = gatewayStore.useStore((s) => [
  ...
  const [isFetching, credentialsLoading] = useLoading(gatewayStore, [
    'getOpenApiConsumerList',
    'getConsumerCredentials',
  ]);
  ...
  const handleDeleteAuthRecord = ({ id, key, name }: any, type: string) => {
    confirm({
      ...
      content: `${i18n.t('msp:confirm to delete this parameter')}：${key || name}`,
      onOk: () => {
      ...
      <div className="auth-params-content">
        <Tabs defaultActiveKey="1" size="small">
          <TabPane
            tab={
              <>
                {i18n.t('msp:key authentication mode')}
                <Popover placement={'top'} content={i18n.t('msp:header-X-App-Key')}>
          ...
          <TabPane tab={`OAuth ${i18n.t('msp:mode')}`} key="2">
            <div className="auth-type-item">
              <h3 className="placeholder" />
          ...
          <TabPane tab={`Hmac ${i18n.t('msp:mode')}`} key="3">
            <div className="auth-type-item">
              <h3 className="placeholder" />
          ...
          <TabPane
            tab={
              <>
                {i18n.t('msp:parameter authentication mode')}
                <Popover placement={'top'} content={i18n.t('msp:appKey-field-request-argument')}>
                  <CustomIcon type="questionfill" className="ml-2 auth-question-icon" />
                </Popover>
              </>
            }
  ...
  const rowSelection = {
    ...
    selections: [
      {
        key: 'all-data',
        text: i18n.t('msp:select all endpoints'),
        onSelect: () => {
      ...
      {
        key: 'invert-all-data',
        text: i18n.t('msp:inverse selection of all endpoints'),
        onSelect: () => {
  ...
  return (
    <div className="consumer-manage">
      ...
      />
      <Drawer
        title={i18n.t('msp:endpoint authorize')}
