...
import { Button, Select, Modal, Input, Spin, Tooltip } from 'core/nusi';
...
const redirectTypeOpt = {
  service: { name: i18n.t('msp:forward by service'), value: 'service' },
  url: { name: i18n.t('msp:forward by address'), value: 'url' },
};
...
export const ApiPackageDetail = () => {
    ...
    ...PACKAGE_DETAIL_COLS,
    {
      ...
      render: (record: any) => (
        <div className="table-operations">
          ...
          >
            {i18n.t('msp:strategy', { packageId: record.id })}
          </span>
          <span
            className="table-operations-btn"
            onClick={() =>
    ...
    service: [
      {
        ...
        itemProps: {
          showSearch: true,
          placeholder: i18n.t('please choose {name}', { name: i18n.t('msp:application name') }),
          onChange: (val: string) => {
            setChosenApp(val);
            setChosenService(undefined);
      ...
      },
      {
        ...
        itemProps: {
          showSearch: true,
          placeholder: i18n.t('please choose {name}', { name: i18n.t('msp:service name') }),
          onChange: (val: string) => {
      ...
      {
        label: i18n.t('msp:service API prefix'),
        name: 'redirectPath',
        type: 'select',
        ...
        itemProps: {
          disabled: !mutable,
          placeholder: i18n.t('msp:splice the URL with the prefix removed, and forward the request'),
  ...
  const fieldsList = [
    ...
    {
      label: i18n.t('msp:URL path prefix'),
      name: 'apiPath',
      rules: [
        {
          pattern: /^\//,
      ...
      itemProps: {
        disabled: !mutable,
        placeholder: i18n.t('msp:match the URL path prefix, and remove the prefix'),
      },
    },
    {
      // 转发方式
      label: i18n.t('msp:forwarding type'),
      name: 'redirectType',
      type: 'radioGroup',
    ...
    ...RedirectTypeFieldMap[chosenRedirectType],
    {
      ...
      type: 'select',
      name: 'method',
      options: [{ name: i18n.t('msp:all methods'), value: 'all' }, ...HTTP_METHODS],
      itemProps: {
        disabled: !mutable || chosenRedirectType === redirectTypeOpt.service.value,
      },
    },
    // {
    //   label: i18n.t('msp:independent authorization method'),
    //   name: 'aclType',
    //   type: 'select',
    //   required: false,
  ...
  if (apiPackageDetail.scene === 'openapi') {
    fieldsList.splice(2, 0, {
      label: i18n.t('msp:skip authentication'),
      initialValue: false,
      name: 'allowPassAuth',
      type: 'switch',
    });
      ...
      <Spin spinning={isFetching}>
        ...
        <div className="mb-4 flex items-center justify-between">
          <div className="api-filter">
            ...
            <Select
              placeholder={i18n.t('msp:api sorting')}
            ...
            <Input
              placeholder={i18n.t('msp:search by API')}
              value={apiPath}
