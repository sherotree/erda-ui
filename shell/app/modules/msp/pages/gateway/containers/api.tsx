...
import { Select, Button, Input, Modal } from 'core/nusi';
import { apiCols, HTTP_METHODS, SORT_FIELDS, SORT_TYPES } from '../config';
import { DomainChecker } from '../components/domain-checker';
import SwaggerDoc from 'msp/pages/gateway/containers/api-doc';
import apiMonitorFilterStore from 'api-insight/stores/filter';
import i18n from 'i18n';
import AddApiModal from 'msp/pages/gateway/components/add-api-modal';
import './api.scss';
...
class API extends React.Component<IProps, IState> {
  ...
  onSaveDomain = () => {
    if (!this.checkDomainInput()) {
      notify('error', i18n.t('msp:a domain name can only contain numbers, lowercase letters, *, -'));
      return;
    }
  ...
  renderSearchConditions = () => {
    ...
    return (
      <div className="condition-nav flex items-center justify-between">
        <div className="app-services-container mb-4">
          <div className={`mr-4 ${isRuntimeEntry ? 'hidden' : ''}`}>
            <span>{i18n.t('msp:switch app')}：</span>
            <Select
              className="api-select"
              placeholder={i18n.t('msp:filter by application')}
              value={diceApp}
          ...
          <div className="mr-5">
            <span>{i18n.t('msp:switch service')}：</span>
            <Select
              className="api-select"
              placeholder={i18n.t('msp:filter by service')}
          ...
          <div className="mr-5">
            <span>{i18n.t('msp:switch deployed branch')}：</span>
            <Select
              className="api-select"
              placeholder={i18n.t('msp:filter by branch')}
          ...
          <IF check={isK8S && currentService}>
            <div className={`${domainSuffix ? '' : 'hidden'}`}>
              <span>{i18n.t('msp:custom domain name')}：</span>
              <Input
              ...
              <IF check={domainPrefix && domainPrefix !== customDomain}>
                <DeleteConfirm
                  title={i18n.t('msp:confirm update operation')}
                  secondTitle={`${i18n.t(
                    'msp:will the current service domain name',
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="search-fields flex items-center justify-between">
            <Select
              allowClear
              className="api-select mr-5"
              placeholder={i18n.t('msp:filter by registration type')}
              value={registerType}
              onChange={this.setRegisterType}
            >
              <Option value="auto">{i18n.t('msp:automatic registration')}</Option>
              <Option value="manual">{i18n.t('msp:manual registration')}</Option>
            </Select>
            {/* <Select allowClear className="api-select mr-5" placeholder={i18n.t('msp:filter by network type')} value={netType} onChange={this.setNetType}>
              <Option value="inner">{i18n.t('msp:internal network')}</Option>
              <Option value="outer">{i18n.t('msp:external network')}</Option>
            </Select> */}
            <Select
              allowClear
              className="api-select mr-5"
              placeholder={i18n.t('msp:filter by calling method')}
              value={method}
            ...
            <Input
              ...
              placeholder={i18n.t('msp:Please enter API to search.')}
              value={apiPath}
    ...
    return (
      <>
        <div className="top-button-group">
          ...
          >
            {i18n.t('msp:add {scope} API', { scope: i18n.t('msp:service') })}
          </Button>
        </div>
