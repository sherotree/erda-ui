...
import { Table, Button, Modal, Drawer, Select, Spin, Tooltip } from 'core/nusi';
...
class InvokerAuthorization extends React.Component<IProps, IState> {
  ...
  constructor(props: IProps) {
        ...
        render: (redirectUris: string) => {
          ...
          return (
            <Tooltip title={redirectUri}>
              <Copy className="cursor-copy" data-clipboard-tip={i18n.t('msp:redirect address')}>
                {redirectUri}
              </Copy>
            </Tooltip>
          );
    ...
    this.oAuthFieldsList = [
      ...
      {
        label: i18n.t('msp:redirect address'),
        name: 'redirect_uri',
        required: false,
        itemProps: {
          addonBefore: (
    ...
    this.apiCols = [
      {
        ...
        render: (path: string) => (
          <Tooltip title={path}>
            <Copy className="cursor-copy" data-clipboard-tip={i18n.t('msp:interface')}>
              {path}
            </Copy>
          </Tooltip>
        ),
    ...
    this.userAuthedApiCols = [
      ...this.apiCols,
      {
        title: i18n.t('msp:flow control strategy'),
        width: 150,
        render: (record: any) => {
        ...
        render: (record: any) => (
          <div className="table-operations">
            <span className="table-operations-btn" onClick={() => this.handleRemoveAuthedApi(record)}>
              {i18n.t('msp:cancel authorization')}
            </span>
          </div>
        ),
      },
  ...
  handleDeleteUser = ({ consumerId }: any) => {
    confirm({
      ...
      content: i18n.t('msp:If you delete a user, all authorized APIs for that user will be deleted.'),
  ...
  handleRemoveAuthedApi = ({ apiId }: any) => {
    confirm({
      title: i18n.t('msp:confirm to cancel'),
      content: i18n.t('msp:confirm to cancel the authorization of {path}'),
      onOk: () => {
        const { authedApiDataSource, selectedConsumerId } = this.state;
  ...
  handleDeleteKeyAuth = ({ id, key }: any) => {
    confirm({
      title: i18n.t('msp:confirm deletion'),
      content: `${i18n.t('msp:confirm to delete this parameter')}：${key}`,
      onOk: () => {
  ...
  handleDeleteOAuth = ({ id, name }: any) => {
    confirm({
      title: i18n.t('msp:confirm deletion'),
      content: `${i18n.t('msp:confirm to delete this parameter')}：${name}`,
      onOk: () => {
        const { authData } = this.props;
  ...
  renderAuthApiContent() {
    ...
    return (
      <div className="auth-api-content">
        <div className="auth-type-item">
          <h3 className="auth-params-title">{i18n.t('msp:authorized interface (configurable flow control policy)')}</h3>
          <Table
        ...
        <div className="auth-type-item">
          <h3 className="auth-params-title">{i18n.t('msp:find the authorization interface')}</h3>
          <SearchTable
  ...
  render() {
    ...
    return (
      <div className="invoker-auth">
        ...
        <Drawer
          title={i18n.t('msp:authorization parameters')}
        ...
        <Drawer
          title={i18n.t('msp:authorization interface')}
